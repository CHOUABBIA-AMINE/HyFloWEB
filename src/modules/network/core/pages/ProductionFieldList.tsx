/**
 * ProductionField List Page - SERVER-SIDE PAGINATION
 * 
...
const ProductionFieldList = () => {
  ...
  useEffect(() => { loadFields(); }, [paginationModel, sortModel, searchText]);
  ...
  const loadFields = async () => {
    try {
      ...
      const pageResponse = searchText 
        ? await ProductionFieldService.globalSearch(searchText, pageable)
        : await ProductionFieldService.getAll(pageable);
        
      setFields(pageResponse.content);
      setTotalRows(pageResponse.totalElements);
      setError('');
    } catch (err: any) {
      setError(err.message || 'Failed to load production fields');
      setFields([]);
      setTotalRows(0);
    } finally {
      setLoading(false);
      ...
      renderCell: (params) => (
        <Box sx={{ display: 'flex', gap: 0.5 }}>
          <Tooltip title={t('common.edit')}>
            <IconButton size="small" onClick={() => navigate(`/network/core/production-fields/${params.row.id}/edit`)} sx={{ color: 'primary.main' }}><EditIcon fontSize="small" /></IconButton>
          </Tooltip>
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this production field?')) {
      try { 
        await ProductionFieldService.delete(id); 
        setSuccess('Production field deleted successfully'); 
        loadFields(); 
      } catch (err: any) { 
        setError(err.message || 'Failed to delete production field'); 
      }
    }
  };
    ...
    <Box>
      <Box sx={{ mb: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
          <Typography variant="h4" fontWeight={700}>Production Fields</Typography>
          <Stack direction="row" spacing={1.5}>
            <Tooltip title={t('common.refresh')}>
              <IconButton onClick={loadFields} color="primary"><RefreshIcon /></IconButton>
            </Tooltip>
            <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate('/network/core/production-fields/create')}>
              Create Production Field
            </Button>
          </Stack>
        </Box>
        <Typography variant="body2" color="text.secondary">
          Manage hydrocarbon extraction sites
        </Typography>
      </Box>
      ...
      <Paper elevation={0} sx={{ mb: 3, border: 1, borderColor: 'divider', p: 2.5 }}>
        <Stack spacing={2.5}>
          <Stack direction="row" spacing={2}>
            <TextField fullWidth placeholder="Search production fields..." value={searchText} onChange={(e) => setSearchText(e.target.value)} InputProps={{ startAdornment: <InputAdornment position="start"><SearchIcon /></InputAdornment> }} sx={{ maxWidth: 400 }} />
...
export default ProductionFieldList;