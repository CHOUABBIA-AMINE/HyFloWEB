...
const BASE_URL = '/api/network/type/vendor-types';
...
export class VendorTypeService {
  ...
  static async getAll(pageable: Pageable): Promise<Page<VendorTypeDTO>> {
    const response = await apiClient.get<Page<VendorTypeDTO>>(BASE_URL, {
      params: {
        page: pageable.page,
        size: pageable.size,
        sort: pageable.sort,
  ...
  static async getById(id: number): Promise<VendorTypeDTO> {
    const response = await apiClient.get<VendorTypeDTO>(`${BASE_URL}/${id}`);
    return response.data;
  }
  ...
  static async create(dto: VendorTypeDTO): Promise<VendorTypeDTO> {
    const response = await apiClient.post<VendorTypeDTO>(BASE_URL, dto);
    return response.data;
  }
  ...
  static async update(id: number, dto: VendorTypeDTO): Promise<VendorTypeDTO> {
    const response = await apiClient.put<VendorTypeDTO>(`${BASE_URL}/${id}`, dto);
    return response.data;
  }
  ...
  static async globalSearch(
    ...
    const response = await apiClient.get<Page<VendorTypeDTO>>(`${BASE_URL}/search`, {
      params: {
        q: searchTerm,
        page: pageable.page,
        size: pageable.size,
