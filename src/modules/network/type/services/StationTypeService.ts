...
export class StationTypeService {
  ...
  /**
   * Get all station types without pagination
   */
  static async getAllNoPagination(): Promise<StationTypeDTO[]> {
    const response = await axiosInstance.get<StationTypeDTO[]>(`${BASE_URL}/all`);
    return response.data;
  }
