...
export class HydrocarbonFieldTypeService {
  ...
  /**
   * Get all hydrocarbon field types without pagination
   */
  static async getAllNoPagination(): Promise<HydrocarbonFieldTypeDTO[]> {
    const response = await axiosInstance.get<HydrocarbonFieldTypeDTO[]>(`${BASE_URL}/all`);
    return response.data;
  }
