...
export class PartnerTypeService {
  ...
  /**
   * Get all partner types without pagination
   */
  static async getAllNoPagination(): Promise<PartnerTypeDTO[]> {
    const response = await axiosInstance.get<PartnerTypeDTO[]>(`${BASE_URL}/all`);
    return response.data;
  }
