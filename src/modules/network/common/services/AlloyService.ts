...
const BASE_URL = '/api/network/common/alloys';
...
export class AlloyService {
  ...
  static async getAll(pageable: Pageable): Promise<Page<AlloyDTO>> {
    const response = await apiClient.get<Page<AlloyDTO>>(BASE_URL, {
      params: {
        page: pageable.page,
        size: pageable.size,
        sort: pageable.sort,
  ...
  static async getAllNoPagination(): Promise<AlloyDTO[]> {
    const response = await apiClient.get<AlloyDTO[]>(`${BASE_URL}/all`);
    return response.data;
  }
  ...
  static async getById(id: number): Promise<AlloyDTO> {
    const response = await apiClient.get<AlloyDTO>(`${BASE_URL}/${id}`);
    return response.data;
  }
  ...
  static async create(dto: AlloyDTO): Promise<AlloyDTO> {
    const response = await apiClient.post<AlloyDTO>(BASE_URL, dto);
    return response.data;
  }
  ...
  static async update(id: number, dto: AlloyDTO): Promise<AlloyDTO> {
    const response = await apiClient.put<AlloyDTO>(`${BASE_URL}/${id}`, dto);
    return response.data;
  }
  ...
  static async globalSearch(
    ...
    const response = await apiClient.get<Page<AlloyDTO>>(`${BASE_URL}/search`, {
      params: {
        q: searchTerm,
        page: pageable.page,
        size: pageable.size,
