/**
 * Location List Page - SIMPLIFIED PATTERN - SERVER-SIDE SEARCH ONLY
 *
 * Features:
...
const LocationList = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const pageResponse = searchText
        ? await LocationService.globalSearch(searchText, pageable)
        : await LocationService.getAll(pageable);

      setLocations(pageResponse.content);
  ...
  const columns: GridColDef[] = useMemo(
    () => [
      ...
      {
        field: 'latitude',
        headerName: t('common.fields.latitude', 'Latitude'),
        width: 120,
        renderCell: (params) => (
          <Typography variant="body2" color="text.secondary">
      ...
      {
        field: 'longitude',
        headerName: t('common.fields.longitude', 'Longitude'),
        width: 120,
        renderCell: (params) => (
          <Typography variant="body2" color="text.secondary">
            {params.value?.toFixed(6) || '-'}
        ...
        renderCell: (params) => (
          <Box sx={{ display: 'flex', gap: 0.5 }}>
            <Tooltip title={t('action.edit', 'Edit')}>
              <IconButton
                size="small"
                onClick={() => navigate(`/general/localization/locations/${params.row.id}/edit`)}
  ...
  return (
    <Box>
      <Box sx={{ mb: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
          ...
          <Stack direction="row" spacing={1.5}>
            ...
            <Button
              variant="contained"
              startIcon={<AddIcon />}
              onClick={() => navigate('/general/localization/locations/create')}
        ...
        <Typography variant="body2" color="text.secondary">
          {t('location.subtitle', 'Manage geographic locations')}
        </Typography>
      </Box>
